...
#' @importFrom lubridate ymd_hms is.POSIXct is.Date
#' @import dplyr
#' @export
  ...
  setNames(lapply(seq_along(db), function(i) {
    ...
    if (nrow(pltTbl) == 0) {
      ...
      if (any(!plt_cols %in% names(pltTbl))) {
        message(paste0("plt_col names not found in ", names(db)[i], " plot table. Missing cols: ", plt_cols[which(!plt_cols %in% names(pltTbl))], "\n"))
      }
      ...
      # merge with data
      db[[i]]$data <- setNames(lapply(seq_along(db[[i]]$data), function(j) {
